...
def separate_materials(model: Path, output: Path) -> dict:
    ...
    _write_json(output / "input.json", {"model": str(model.resolve()), "output": str(output.resolve()),
                                      "source_sha256": _digest(model)})
    ...
    result = {"source_sha256": _digest(model), "model_sha256": _digest(output / "character.glb"),
              "recipe": "material-boundaries-v1", "status": "review_required"}
