...
const output = path.resolve(root, '../backend/assets/avatars/manual-v1');
const io = new NodeIO();
const absolute = {};
...
async function build(def, lod) {
  ...
  return {
    source: { uri: `/api/avatars/assets/${def.id}/model?lod=${lod}` },
...
for (const def of definitions) {
  ...
  records.push({
    ...(['body', 'hair', 'hat', 'top', 'bottom', 'shoes', 'face'].includes(def.slot) ? { slot: def.slot } : {}),
    id: def.id,
    name: def.name,
    kind: 'characterPart',
...
await fs.writeFile(
  ...
  JSON.stringify(
    {
      generator: 'frontend/scripts/build-avatar-fixtures.mjs',
      license: 'MIT',
