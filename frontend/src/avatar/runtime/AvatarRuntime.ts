...
import { AnimationSystem } from './animation-controller';
import {
  GLTFAssetCache,
  gltfAssetCache,
  type GLTFAssetLease,
} from '../../assets/GLTFAssetCache';
...
export class AvatarRuntime {
  ...
  private rigId: AvatarRigId = 'gaesup-humanoid-v1';
  get rig(): AvatarRigId { return this.rigId; }
  private readonly root = new Group();
  private master = createAvatarSkeleton(this.root);
  ...
  private async apply(input: AvatarState, level = this.level): Promise<void> {
    ...
    if (!this.state.body && body.rig !== this.rig) {
      // Select a registered rest pose before the first asset is bound. Existing
      // outfits keep their skeleton; incompatible body swaps fail atomically.
      if (manifests.some(manifest => manifest.rig !== body.rig))
        throw new AvatarCompatibilityError('Rig/body archetype mismatch');
      this.master.bones.root.removeFromParent();
      this.master.skeleton.dispose();
      this.rigId = body.rig;
      this.master = createAvatarSkeleton(this.root, body.rig);
    }
