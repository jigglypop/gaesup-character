...
import maple from '../core/rig-maple.json';
...
export function createAvatarSkeleton(root: Group, rig = definition.id) {
  const bones = {} as Record<HumanoidBone, Bone>;
  const selected = rig === maple.id ? maple : definition;
  for (const [key, parent, x, y, z] of selected.bones) {
    const bone = new Bone();
