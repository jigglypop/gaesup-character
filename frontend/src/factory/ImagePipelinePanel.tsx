...
const characterSlots=['body','hair','hat','top','bottom','shoes'];
const names: Record<string,string> = {body:'대머리 기본몸 · 얼굴 포함',face:'얼굴',hair:'머리카락',hairBack:'뒷머리 · 숨은 두피 포함 후보',hairFront:'앞머리 · 얼굴 둘레',hat:'모자',top:'상의',bottom:'하의',shoes:'신발 한 쌍',weapon:'무기',shield:'방패',back:'등 장비',faceAccessory:'얼굴 장식',neckAccessory:'목 장식'};
...
export function ImagePipelinePanel({blueprint,onBlueprint,dirty}:{blueprint:Blueprint;onBlueprint:(b:Blueprint)=>void;dirty:boolean}) {
  ...
  const reusable=jobs.filter(j=>j.source_sha256===blueprint.source_sha256&&j.parts?.some(p=>characterSlots.includes(p.slot)&&p.slot!=='body'&&p.image_status==='succeeded'&&p.model_status==='ready'));
  const reuseSource=pending
    ? jobs.find(j=>j.id===pending.input.reuse_job_id)
    : reusable.find(j=>j.id===reuseJobId)||reusable[0];
  const reusableSlots=new Set(reuseSource?.parts?.filter(p=>characterSlots.includes(p.slot)&&p.slot!=='body'&&p.image_status==='succeeded'&&p.model_status==='ready').map(p=>p.slot)||[]);
  ...
  const refresh=useCallback(async()=>{
    try {
      ...
      setJobs(related);setReuseJobId(current=>current||related.find(j=>j.source_sha256===blueprint.source_sha256&&j.parts?.some(p=>characterSlots.includes(p.slot)&&p.slot!=='body'&&p.image_status==='succeeded'&&p.model_status==='ready'))?.id||'');
  ...
  return <section className="image-production" aria-label="캐릭터 파츠 생산">
    <div className="image-production-title"><div><h2>한 캐릭터 파츠 전체 분리</h2><p>원본 캐릭터 디자인을 기본몸·머리카락·모자·상의·하의·신발로 나눠 각각 이미지와 3D를 만듭니다.</p></div><span className="image-production-config">{capabilities?.image_model||'GPT Image 2.5'} {capabilities?.image_configured?'연결 설정됨':'설정 필요'} · Meshy {capabilities?.meshy_configured?'연결 설정됨':'설정 필요'} · Blender {capabilities?.blender_available?'준비됨':'설치 필요'}</span></div>
    <div className="image-production-options"><label>생산 방식<select aria-label="생산 방식" value={requestedMode} disabled={busy||active||!!pending} onChange={e=>setProductionMode(e.target.value as typeof productionMode)}><option value="character_parts">기본 · 캐릭터 6파트 한 세트</option><option value="legacy">고급 · 기존 단일 몸/선택 파츠</option></select></label></div>
    {requestedMode==='character_parts'?<>
      <div className="image-production-slots" aria-label="기본 6파트">{characterSlots.map(slot=><label key={slot}><input type="checkbox" checked readOnly disabled/>{names[slot]}</label>)}</div>
    ...
    {job&&<div className="image-production-job"><div className="image-production-title"><select aria-label="이미지 생산 버전" value={job.id} onChange={e=>setChosen(e.target.value)}>{jobs.map(j=><option key={j.id} value={j.id}>{new Date(j.created_at).toLocaleString()} · {j.id.slice(0,8)}</option>)}</select><strong role="status">{job.status==='review_required'?'생성 결과 · 검수 대기':job.progress.message}</strong></div>
      {characterJob&&<p>원본 이미지와 아래 파츠 결과를 비교해 얼굴·실루엣·머리카락·모자 분리·의상 절개·가려졌던 면의 생성 보완을 각각 검수하세요.</p>}
