...
export function NativeAssembly({ jobId, simple = false, flow }: { jobId: string; simple?: boolean; flow?: FactoryJob['character_flow'] }) {
  ...
  const available = state?.status === 'review_required' && !!state.version;
  return <section className="native-assembly" data-assembly-job={jobId}>
    {(error || polling.error) && <p role="alert">{error || polling.error}</p>}
    {available ? <NativeCharacter key={`${jobId}:${state.version}`} jobId={jobId} state={state} /> : <div className="meshy-motion">
      ...
      {!simple && <button disabled={busy || !state || ['accepted', 'running'].includes(state.status)} onClick={() => void assemble()}>파츠 조립 · 로컬 처리</button>}
      {state && ['accepted', 'running'].includes(state.status) && <p role="status">파츠를 공통 골격에 연결하고 있습니다…</p>}
      {state?.error && <p role="alert">{state.error}</p>}
    </div>}
    {!simple && available && <button className="assembly-tools" disabled={busy || !!flow?.busy} onClick={() => void assemble()}>{busy ? '조립 중…' : state.fit_update_available ? '공통 규격으로 맞추기' : '파츠 다시 맞추기'}</button>}
    {!simple && available && <button className="assembly-tools" aria-expanded={rigControls} onClick={() => setRigControls(value => !value)}>몸 리깅·추가 동작 설정</button>}
    {!simple && (!available || rigControls) && <MeshyMotion jobId={jobId} />}
  </section>;
}
...
function NativeCharacter({ jobId, state }: { jobId: string; state: NativePartsState }) {
  ...
  const [ready, setReady] = useState(false), [wearing, setWearing] = useState(false), [attempt, setAttempt] = useState(0);
  ...
  useEffect(() => {
    setReady(false); setModelError(''); applied.current = [];
    ...
    void instance.load(body.url, { sha256: body.sha256, wardrobe: true }).then(value => {
      ...
      setClips(value);
      const start = -1;
      instance.play(start); setMotion(start); setReady(true);
  ...
  async function save() {
    ...
    const request = readPending(storage) || { key: crypto.randomUUID(), revision, input: { body_sha256: body.sha256, slots: [...applied.current] } };
    try {
      ...
      if (alive.current) { setRevision(result.revision); setSaved(result.slots); setSelected(result.slots); }
    } catch (e) {
  ...
  return <section ref={panel} className="meshy-motion assembly-preview" data-assembly-ready={appliedSelection && !wearError ? version : ''}>
    ...
    <div className="meshy-buttons"><button disabled={busy || !appliedSelection || !!wearError || (!pending && equal(selected, saved) && revision !== '0')} onClick={() => void save()}>{pending ? '조합 저장 결과 복구' : '현재 조합 저장'}</button><button disabled={busy || pending} onClick={() => void restore()}>저장한 조합 다시 불러오기</button></div>
    {restored && !pending && revision !== '0' && equal(selected, saved) && <p role="status">저장된 조합입니다.</p>}
