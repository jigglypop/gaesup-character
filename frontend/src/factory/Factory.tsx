...
function Result({ job, bodyOnly, assets }: { job: FactoryJob; bodyOnly: boolean; assets: AssetRecord[] }) {
  ...
  return <div className="factory-result" data-factory-ready={runtime ? job.id : ''}>
    ...
    {!bodyOnly && <div className="factory-swaps"><strong>생산한 파츠 교체</strong>{Object.entries(names).filter(([slot]) => slot !== 'body' && assets.some(asset => asset.metadata?.['factory'] && (asset.metadata?.['avatar'] as { slot: string }).slot === slot)).map(([slot, label]) => <label key={slot}>{label}<select aria-label={`생산 ${label} 교체`} disabled={!runtime || busy} value={state.equipment[slot as EquipmentSlot] || ''} onChange={event => void equip(slot as EquipmentSlot, event.target.value)}><option value="">장착 해제</option>{assets.filter(asset => asset.metadata?.['factory'] && (asset.metadata?.['avatar'] as { slot: string }).slot === slot).map(asset => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select></label>)}</div>}
...
function GLBFactory() {
  ...
  const complete = !!job?.outfit && assets.some(asset => asset.id === job.outfit!.body);
  const activeStage = job?.status === 'review_required' ? 6 : stages.findIndex(([id]) => id === job?.progress.stage);
  return <div className="factory-shell">
    ...
    <aside className="factory-sidebar"><span className="factory-eyebrow">SOURCE CHARACTERS</span><h2>캐릭터 투입</h2><p>Meshy·Tripo·Blender에서 받은 캐릭터를 같은 규격의 아바타로 만듭니다.</p>
      ...
      <div className="factory-profile"><span className="factory-eyebrow">COMMON BODY STANDARD</span><h3>{job?.profile?.name || 'Maple SD'}</h3><svg viewBox="0 0 200 220" aria-label="공통 몸 비율 설계도"><g fill="#c9dcc0" stroke="#d5f4ae" strokeWidth="1.3"><ellipse cx="100" cy="51" rx="41" ry="44" /><path d="M85 93 Q100 88 115 93 L122 133 Q100 146 78 133Z" /><ellipse cx="71" cy="119" rx="10" ry="25" transform="rotate(29 71 119)" /><ellipse cx="129" cy="119" rx="10" ry="25" transform="rotate(-29 129 119)" /><rect x="80" y="136" width="17" height="54" rx="8"/><rect x="103" y="136" width="17" height="54" rx="8"/></g><g stroke="#809671" strokeDasharray="3 3"><path d="M25 8H175M25 96H175M25 194H175M158 8V194"/></g><g fill="#9cab91" fontSize="9"><text x="165" y="60">HEAD</text><text x="165" y="152">BODY</text></g></svg><dl><div><dt>비율</dt><dd>약 2.45등신</dd></div><div><dt>공통 리그</dt><dd>23 bones · A-pose</dd></div><div><dt>몸</dt><dd>새 공통 템플릿</dd></div></dl></div>
    </aside>
    <main className="factory-main"><div className="factory-heading"><div><span className="factory-eyebrow">CHARACTER → MODULAR AVATAR</span><h1>{source?.name || '아바타 공장'}</h1><p>기본 몸 설계부터 의상 분리, 공통 리깅, 교체 가능한 파츠 출력까지.</p></div><button className="factory-primary" disabled={busy || (!source?.model_id && !pending) || (!pending && ['accepted', 'running'].includes(job?.status || ''))} onClick={() => void produce(source ? [source.id] : [])}>{busy ? '접수 중…' : pending ? '생산 요청 복구' : job ? '새 버전 생산' : '모듈형 아바타 생산'}</button></div>
      <ol className="factory-stages">{stages.map(([id, label], index) => <li key={id} className={index < activeStage ? 'done' : index === activeStage ? 'active' : ''}><span>{index < activeStage ? '✓' : String(index+1).padStart(2, '0')}</span><strong>{label}</strong></li>)}</ol>
      ...
      {job && <div className="factory-job-status"><span className={`factory-job-dot ${job.status}`} /><strong>{statusNames[job.status] || job.status}</strong><span>{job.status === 'review_required' ? '공통 몸·파츠 출력 완료. 외형과 변형을 확인하세요.' : job.error || job.progress.message}</span><select aria-label="생산 버전" value={job.id} onChange={event => { setJobId(event.target.value); setTab('source'); }}>{versions.map(v => <option key={v.id} value={v.id}>{new Date(v.created_at).toLocaleString()} · {statusNames[v.status]}</option>)}</select></div>}
      <section className="factory-workspace"><div className="factory-tabs"><button aria-pressed={tab === 'source'} onClick={() => setTab('source')}>원본 · 분리 영역</button><button aria-pressed={tab === 'body'} disabled={!complete} onClick={() => setTab('body')}>공통 기본 몸</button><button aria-pressed={tab === 'result'} disabled={!complete} onClick={() => setTab('result')}>생산 결과 · 파츠 교체</button><span>{selections.reduce((n, s) => n+s.faces.length, 0)}개 보정 면</span></div>
        {source && tab === 'source' && <SourceEditor key={source.id} character={source} onSelections={setSelections} />}
        {complete && tab !== 'source' && <Result key={`${job.id}:${tab}`} job={job} assets={assets} bodyOnly={tab === 'body'} />}
        {!source && <div className="factory-empty">캐릭터 GLB를 가져오면 생산을 시작할 수 있습니다.</div>}
      </section>
      {job?.technical && <section className="factory-output"><div><span className="factory-eyebrow">PRODUCTION PACKAGE</span><h2>같은 몸, 같은 리그, 교체 가능한 파츠</h2><p>{job.technical.bone_count}개 공통 뼈 · {job.technical.parts}개 파츠 · 원본 보존 · 외형 검수 대기</p>{job.technical.file_bytes!==undefined&&<p>조립 파일 {(job.technical.file_bytes/1024/1024).toFixed(2)} MiB · 텍스처 {((job.technical.texture_pixels||0)/1e6).toFixed(1)} MP · 원본 해상도 유지{job.technical.resource_warnings?.length?' · 권장 용량 초과는 안내로만 표시합니다.':''}</p>}</div><div className="factory-downloads">{job.artifacts.filter(a => ['master.blend', 'character.glb', 'body.glb', 'compilation.json'].includes(a.name)).map(a => <a key={a.name} href={a.url} download><StudioIcon name="download" size={16} />{a.name}</a>)}</div><div className="factory-renders">{job.artifacts.filter(a => a.name in reviewNames).map(a => <a key={a.name} href={a.url} target="_blank" rel="noreferrer"><img src={a.url} alt={reviewNames[a.name]} /><span>{reviewNames[a.name]}</span></a>)}</div><div className="factory-limitations">{job.evidence?.limitations.map(value => <p key={value}>{value}</p>)}</div></section>}
    </main>
  </div>;
}
